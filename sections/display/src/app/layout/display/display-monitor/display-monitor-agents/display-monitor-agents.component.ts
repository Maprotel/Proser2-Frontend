...
import { DisplayMonitorByAgentsService } from "sections/display/src/shared/services";
import { DisplayMonitorByAgentsModel } from "sections/display/src/shared/models";
...
@Component({
  selector: "app-display-display-monitor-agents",
  templateUrl: "./display-monitor-agents.component.html",
  styleUrls: ["./display-monitor-agents.component.scss"]
})
export class DisplayMonitorAgentsComponent implements OnInit {
  // Subscription
  ...
  ngOnInit() {
    ...
    this.setReportTitles("Monitor de agentes");
    this.onRepeat();
  }
  ...
  getMonitorList(userSelection: UserSelectionModel) {
    ...
    this.displayMonitorByAgentsService.getReportList(userSelection).subscribe(
      res => {
        this.timerConnected = 0;
        if (res) {
