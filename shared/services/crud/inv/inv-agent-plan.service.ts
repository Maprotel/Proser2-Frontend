...
import { InvAgentPlanModel } from "shared/models";

@Injectable({
  providedIn: "root"
})
export class InvAgentPlanService {
  api_string = "/api/AgentPlans";
  ...
  getAllRecords(query?): Observable<InvAgentPlanModel[]> {
  ...
  postRecord(item?) {
    ...
    return this.http
      .post<InvAgentPlanModel>(url_api, query, { headers: this.headers })
      .pipe(map(data => data));
  }
  ...
  putRecord(item?) {
    ...
    return this.http
      .put<InvAgentPlanModel>(url_api, query, { headers: this.headers })
      .pipe(map(data => data));
  }
  ...
  deleteRecord(id) {
    const accessToken = localStorage.getItem("accessToken");
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?&access_token=${accessToken}`;
    return this.http.delete<InvAgentPlanModel>(url_api, id).pipe(map(data => data));
  }

  getRecordById(id) {
    const accessToken = localStorage.getItem("accessToken");
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
    return this.http.get<InvAgentPlanModel>(url_api, id).pipe(map(data => data));
  }
